...
# --- APP CONFIG (Fixes upload limits) ---
# Streamlit usually limits to 200MB, which is plenty for SOFs.
...
def tdc_final_engine():
    ...
    with st.sidebar:
        ...
        # DESPATCH RULE: HALF OF DEMURRAGE
        ...
        nor_rule_type = st.selectbox("NOR Rule Type", [
            "12 Hour Turn Time", "24h Turn Time", "LAFAMA Rule", "6:00 PM / 8:00 AM Rule"
        ])

    # --- 2. UPLOADER FIX ---
    st.subheader("📂 Document Management")
    uploaded_file = st.file_uploader("Upload SOF (PDF, Image, or Excel)", type=["pdf", "jpg", "jpeg", "png", "xlsx"])
    
    if uploaded_file is not None:
        st.success(f"✅ {uploaded_file.name} uploaded successfully.")
        # Logic to process file would go here; currently serves as a reference.
    ...
    with col2:
        ...
        st.caption("Include Cleaning, Surveys, and Weather here.")
    ...
    if rate > 0:
        ...
        else: tt_exp = nor_t # Simplified for this view

        # Engine Logic (1-min precision)
        total_allowed_sec = (qty / rate) * 86400
        curr = max(tt_exp, ops_s) # Laytime Start
        l_start_display = curr
        used_sec = 0
        ...
        while used_sec < total_allowed_sec:
            ...
            # Keyword Deductions (Cleaning, Survey, Berthing, Weather)
            if not excluded:
        ...
        share_col.write(f"**Expiry Wall:** {curr.strftime('%d-%b %H:%M')}")
        
        # Simple export for clients
        st.download_button("Export Calculation (CSV)", data=sof_events.to_csv(), file_name=f"{vessel_name}_statement.csv")
